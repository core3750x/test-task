...
- перезагружает сервер после изменения правила, если `network_rename_reboot: true`;
...
Перезагрузка выполняется только если конфигурация переименования изменилась. Её можно отключить переменной `network_rename_reboot: false`, но тогда новое имя применится после ручной перезагрузки или повторного создания интерфейса.
