...
После выполнения роль создаёт запись в `/etc/crypttab` без keyfile. Поэтому после перезагрузки система может запросить passphrase для открытия контейнера. Файловая система монтируется в указанную точку, по умолчанию `/data`.
